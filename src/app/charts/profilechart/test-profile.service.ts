...
export class TestPG_Service {
  ...
  async renderLayout() {
    ...
    if (this.weeklyChart) {
      ...
      this.last_friday = new Date();
      this.last_friday.setDate(
        this.last_friday.getDate() - ((this.last_friday.getDay() + 2) % 7)
      );
      this.last_friday.setHours(0);
      this.last_friday.setMinutes(0);
      this.last_friday.setSeconds(0);
      this.last_friday.setMilliseconds(0);
    ...
    $('.interval').css('display', 'flex');

    if (this.socketBool) {
      this.connectSocket(vpoc_va_arr);
    }
  }
  ...
  async connectSocket(array: any) {
    ...
    setInterval(() => {
      ...
      let lastEle = this.chartData[this.chartData.length - 1];

      this.chartData[this.chartData.length - 1].total_volume +=
        data.lastTradedQuantity;

      if (lastEle['ohlc'].high < data['ohlc'].high) {
        this.chartData[this.chartData.length - 1]['ohlc'].high =
          data['ohlc'].high;
      }
      if (lastEle['ohlc'].low > data['ohlc'].low) {
        this.chartData[this.chartData.length - 1]['ohlc'].low =
          data['ohlc'].low;
      }
      if (lastEle['valueArea'].vah < data['valueArea'].vah) {
        this.chartData[this.chartData.length - 1]['valueArea'].vah =
          data['valueArea'].vah;
      }
      if (lastEle['valueArea'].val > data['valueArea'].val) {
        this.chartData[this.chartData.length - 1]['valueArea'].val =
          data['valueArea'].val;
      }
