...
export class TestingService {
  ...
  updateTimeWiseDataMapping(chartData: any) {
    this.timeRangeData.forEach(async (element) => {
      ...
      let total_volume: any = Object.values(this.vahvalValue).reduce(
  ...
  async renderLayout(chartData: any) {
    ...
    if (this.socketBool == true && !this.start_socket_connection) {
      ...
      this.connectSocket(this.commonSer.vpoc_valueArea(chartData),chartData.length);
    } else {
  ...
  newInterVal:any ;
  async connectSocket(array: any , count:number) {
